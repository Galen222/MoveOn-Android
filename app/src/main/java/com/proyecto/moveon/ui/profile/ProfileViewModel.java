    ...
    public void logout() {
        ...
        authRepository.logout(refreshToken, new AuthRepository.Callback<String>() {
            ...
            public void onSuccess(String result) {
                performLocalLogout();
                logoutState.postValue(UiState.success(result)); // ¡Éxito!
            }

            @Override
            public void onError(String error) {
                performLocalLogout();
                logoutState.postValue(UiState.error(error)); // Falló el servidor, emitimos error
            }
        });
    }

    private void performLocalLogout() {
        // Aprovechamos el método seguro que arreglamos antes
